...
// Only add event listeners if elements exist (checkout page specific)
if (paypalOption && creditCardOption) {
    paypalOption.addEventListener('change', updatePaymentMethod);
    creditCardOption.addEventListener('change', updatePaymentMethod);
    updatePaymentMethod();
}

if (cardNumberInput && expiryDateInput && cvcInput) {
...
function updatePaymentMethod() {
	if (!paypalOption || !creditCardOption || !creditCardForm) return;
...
function handleCardNumberInput(e) {
	if (!cardNumberInput || !cardType || !cardError) return;
