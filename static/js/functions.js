(function ($) {
	...
	function lynessa_view_mode_sidebar () {
		$('.has-sidebar .grid-view-mode .modes-mode').on('click', function (e) {
			...
			if ($this.hasClass('mode-grid')) {
				$('.product-item').addClass(
					'col-bg-4 col-xl-4 col-lg-6 col-md-6 col-sm-6 col-ts-6 style-01').removeClass(
					'col-md-12 list');
			}
			else {
				$('.product-item').removeClass(
					'col-bg-4 col-xl-4 col-lg-6 col-md-6 col-sm-6 col-ts-6 style-01').addClass(
					'col-md-12 list');
			}
	...
	function lynessa_view_mode_nosidebar () {
		$('.no-sidebar .grid-view-mode .modes-mode').on('click', function (e) {
			...
			if ($this.hasClass('mode-grid')) {
				$('.product-item').addClass(
					'col-bg-3 col-xl-3 col-lg-4 col-md-6 col-sm-6 col-ts-6 style-01').removeClass(
					'list');
			}
			else {
				$('.product-item').removeClass(
					'col-bg-3 col-xl-3 col-lg-4 col-md-6 col-sm-6 col-ts-6 style-01').addClass(
					'col-md-12 list');
			}
	...
	function lynessa_title_tooltip () {
		$(".lynessa-tooltip").mousemove(function (event) {
			var relX = event.pageX - $(this).offset().left - $(this).children('.lynessa-tooltip-inner')
				.outerWidth() / 2;
			var relY = event.pageY - $(this).offset().top - $(this).children('.lynessa-tooltip-inner')
				.outerHeight() - 5;
		...
		$elem.find('.vertical-menu').each(function () {
			...
			_main.children('.menu-item.parent').each(function () {
				...
				$(this).children('.toggle-submenu').on('click', function () {
					...
					_main.find('.menu-item.parent').not($(this).parent()).removeClass(
						'show-submenu');
				});
				...
				next_curent.children('.menu-item.parent').each(function () {
					...
					$(this).children('.toggle-submenu').on('click', function () {
						$(this).parent().parent().find('.submenu').not(child_curent)
							.stop().slideUp(300);
						$(this).parent().children('.submenu').stop().slideToggle(
							300);
						$(this).parent().parent().find('.menu-item.parent').not($(
	...
	function lynessa_animation_tabs ($elem, _tab_animated) {
		...
		$elem.find('.owl-slick .slick-active, .product-list-grid .product-item').each(function (i) {
			...
			).addClass(_tab_animated + ' animated').one(
	...
	window.lynessa_init_carousel = ($elem) => {
		...
		$elem.each(function () {
			...
			_main.children('.cat-parent').each(function () {
				...
				next_curent.children('.cat-parent').each(function () {
					...
					$(this).children('.carets').on('click', function () {
						$(this).parent().toggleClass('show-sub');
						$(this).parent().parent().find('.cat-parent').not($(this)
							.parent()).removeClass('show-sub');
						$(this).parent().parent().find('.children').not(
							child_curent).stop().slideUp(300);
	...
	function lynessa_popover_button () {
		...
		$('.product-inner.tooltip-all-top .yith-wcqv-button,.product-inner.tooltip-top .add-to-cart a, .product-inner.tooltip-top .compare,.product-inner.tooltip-top .yith-wcwl-add-to-wishlist a')
			.each(function () {
				$(this).tooltip({
					title: $(this).text(),
					trigger: 'hover',
		...
		$('.product-inner.tooltip-left .add-to-cart a, .product-inner.tooltip-left .yith-wcqv-button,.product-inner.tooltip-left .compare,.product-inner.tooltip-left .yith-wcwl-add-to-wishlist a')
			.each(function () {
				$(this).tooltip({
					title: $(this).text(),
					trigger: 'hover',
		...
		$('.product-inner.tooltip-right .add-to-cart a, .product-inner.tooltip-right .yith-wcqv-button,.product-inner.tooltip-right .compare,.product-inner.tooltip-right .yith-wcwl-add-to-wishlist a')
			.each(function () {
				$(this).tooltip({
					title: $(this).text(),
					trigger: 'hover',
	...
	function lynessa_threesixty (ev) {
		...
		$('.lynessa-threed-view').ThreeSixty({
			...
			progress: '.spinner',
			imgArray: [imageLink + '/is_main.jpg', imageLink + '/is_main1.jpg', imageLink + '/is_main2.jpg',
				imageLink + '/is_main4.jpg', imageLink + '/is_main5.jpg', imageLink + '/is_main6.jpg',
				imageLink + '/is_main7.jpg', imageLink + '/is_main8.jpg', imageLink + '/is_main9.jpg',
				imageLink + '/is_main10.jpg', imageLink + '/is_main11.jpg', imageLink + '/is_main12.jpg'
			],
			height: 600,
			width: 600,
			responsive: true,
